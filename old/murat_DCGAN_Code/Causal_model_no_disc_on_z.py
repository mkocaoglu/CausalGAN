...
########################################################################################        
  ...
  def build_model(self):
    ...
    self.g_loss = self.g_lossGAN - 1.0*self.k_t*self.g_lossLabels_GLabeler + self.g_lossLabels#self.m_t*self.g_loss_label_mix
      ...
      for idx in xrange(0, batch_idxs):
        ...
        #   pairwise(self)
          ...
          #  _,summary_str = self.sess.run([d_optim,self.summary_op],feed_dict = fd)
            ...
            _, _, _, summary_str = self.sess.run([d_label_optim,d_gen_label_optim,d_optim, self.summary_op], feed_dict=fd)
           ...
           # _, = self.sess.run([g_optim], feed_dict=fd)
            ...
            _, _ = self.sess.run([g_optim,k_t_update], feed_dict=fd)
