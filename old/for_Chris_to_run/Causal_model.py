...
class DCGAN(object):
  ...
  def __init__(self, sess, input_height=108, input_width=108, is_crop=True,
    ...
    self.gamma_m = self.label_loss_hyperparameter
    self.lambda_m =  0.01
...
########################################################################################        
  ...
  def build_model(self):
    ...
    self.g_loss = self.g_lossGAN - 2.0*self.k_t*self.g_lossLabels_GLabeler + self.g_lossLabels#self.m_t*self.g_loss_label_mix
    ...
    self.k_t_sum = scalar_summary( 'k_t', self.k_t)
    #self.m_t_sum = scalar_summary( 'm_t', self.m_t)
  ...
  def train(self, config):
    ...
    #m_t_update = tf.assign(self.m_t, tf.clip_by_value(self.m_t + self.lambda_m*(-self.g_lossGAN + self.gamma_m*self.g_loss_label_mix),0.0,1.0))
      ...
      for idx in xrange(0, batch_idxs):
        ...
        #   pairwise(self)
          ...
          #  _,summary_str = self.sess.run([d_optim,self.summary_op],feed_dict = fd)
            ...
            _ = self.sess.run([k_t_update],feed_dict=fd)#,feed_dict = fd)
            #self.writer.add_summary(summary_str, counter)
          #else:
          #  _ = self.sess.run([g_optim], feed_dict=fd)
            #self.writer.add_summary(summary_str, counter)
